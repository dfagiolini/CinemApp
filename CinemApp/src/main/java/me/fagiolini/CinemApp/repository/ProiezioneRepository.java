...
public interface ProiezioneRepository extends JpaRepository<Proiezione, Long> {
    @Query(value = "select p.* from proiezione p , cinema c , sala s , film f where c.id = s.cinema_id and p.sala_id = s.id and f.id = p.film_id and c.id = ?1;", nativeQuery = true)
    List<Proiezione> findProiezioneByCinema(long cinemaId);

    @Query(value = "select p.* from proiezione p , cinema c , sala s , film f where c.id = s.cinema_id and p.sala_id = s.id and f.id = p.film_id and f.id = ?1;", nativeQuery = true)
